...
bool Connection::sendImage(const QByteArray &message)
{
    if (message.isEmpty())
        return false;

    QByteArray data = "IMAGE " + QByteArray::number(message.size()) + ' ' + message;
    return write(data) == data.size();
}
...
bool Connection::sendFile(const QByteArray &message)
{
    ...
    QByteArray data = "FILE " + QByteArray::number(message.size()) + ' ' + message;
    return write(data) == data.size();
}
...
void Connection::processData()
{
    ...
    case PlainText:
        ...
        break;
    case Image:
        emit newImage(username,buffer);
        break;
    case File:
        emit newFile(username,buffer);
        break;
    case Ping:
        write("PONG 1 p");
        break;
